...
def predict_user_user(data_raw,x_raw,N=10,frac=0.02):
    ...
    for movie in ix_raw_test:
        # Your solution here
        sims = db[db.loc[:, movie] != 0].apply(lambda y: (y.values*x.values).sum()/(np.sqrt((y**2).sum())*np.sqrt((x**2).sum())),axis=1)
        sims = sims.dropna()
        try:
...
def predict_item_item(data_raw,x_raw,N=10,frac=0.02,debug={}):
    ...
    for movie in ix_raw_test:
        ...
        top_sims = sorted_sims.iloc[:N]
        ids = top_sims.index
        preds.append(x_raw[ids].mean())
        
        # preds.append(reviews.mean())
    pred = pd.Series(preds,index=x_raw_test.index)
    actual = x_raw_test
    mae = (actual-pred).abs().mean()
    return mae
